...
if __name__ == '__main__':
    ...
    df = data_utils.load_multi_class_anemia_dataset(csv_file)
    ...
    for model in models:
        ...
        output_plot_file = f"multi_class_output/{model}_confusion_matrix.png"
        output_metrics_file = f"multi_class_output/{model}_metrics.png"
        model_metrics = model_utils.plot_confusion_matrix(test_Y=test_Y, y_pred=y_pred, labels=labels, model_name=model,
                                                          ...
                                                          percentage=True, plot_metrics_figure=True)
        metrics_list.append(model_metrics)
        if model == "DecisionTree":
            plt.figure(figsize=(20, 10))
            plot_tree(trained_model, filled=True, feature_names=df.columns, class_names=labels, rounded=True)
            plt.title("Decision Tree")
            plt.savefig(f"multi_class_output/{model}_tree.png")
            plt.close()

    with open("multi_class_output/model_metrics.json", "w") as metrics_json:
